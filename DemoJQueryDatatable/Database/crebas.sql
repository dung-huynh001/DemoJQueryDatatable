...
create table APPOINTMENT (
   ...
   APPOINTMENTNAME      varchar(256)         not null,
   DATEOFCONSULTATION   datetime             not null,
   APPOINTMENTDATE      datetime             not null,
   APPOIMENTSTATUS      varchar(256)         not null,
   CLOSEDDATE           datetime             null,
   CLOSEDBY             varchar(256)         null,
   SYMTOMS              varchar(256)         null,
   EXISTIONGILLNESS     varchar(256)         null,
   DRUGALLERGLES        varchar(256)         null,
   NOTE                 varchar(256)         null,
   CASENOTE             varchar(256)         null,
   DIAGNOSIS            varchar(256)         null,
   ADVICETOPATIENT      varchar(256)         null,
   LABTESTS             varchar(256)         null,
...
create table CONSULTANT_TYPE (
   ...
   CONSULTANTTYPENAME   char(256)            not null,
   CREATEDBY            varchar(50)          null,
   CREATEDDATE          datetime             null,
   UPDATEDBY            varchar(50)          null,
   UPDATEDDATE          datetime             null,
...
create table MODE_OF_CONSULTING (
   ...
   UPDATEDDATE          datetime             null,
   DELETEDFLAG          bit                  not null,
   constraint PK_MODE_OF_CONSULTING primary key nonclustered (MODE_ID)
)
go
...
create table QUALIFICATION (
   ...
   DOCTORID             int                  not null,
   QUALIFICATIONNAME    varchar(256)         not null,
   CREATEDBY            varchar(50)          null,
   CREATEDDATE          datetime             null,
   UPDATEDBY            varchar(50)          null,
   UPDATEDDATE          datetime             null,
...
alter table APPOINTMENT
   add constraint FK_APPOINTM_RELATIONS_MODE_OF_ foreign key (MODE_ID)
      references MODE_OF_CONSULTING (MODE_ID)
go
...
WHILE @patient <= 20
BEGIN
 INSERT INTO [USER] VALUES(1302, 'patient'+CONVERT(VARCHAR, @patient), '1234', 'patient'+CONVERT(VARCHAR, @patient)+'@gmail.com', GETDATE(), 'Patient', NULL, 'passwrdrecovery1', 'passwrdrecovery2', 'active', 5, NULL, 'Admin', GETDATE(), 'Admin', GETDATE(), 'False')
...
WHILE @doctor <= 20
BEGIN
 INSERT INTO [USER] VALUES(1303, 'doctor'+CONVERT(VARCHAR, @doctor), '1234', 'doctor'+CONVERT(VARCHAR, @doctor)+'@gmail.com', GETDATE(), 'Patient', NULL, 'passwrdrecovery1', 'passwrdrecovery2', 'active', 5, NULL, 'Admin', GETDATE(), 'Admin', GETDATE(), 'False')
 SET @doctor = @doctor + 1;
END;
...
WHILE @cnt <= 100
BEGIN
 INSERT INTO [USER] VALUES(CONVERT(INT,FLOOR(RAND()*(1305-1301+1)+1301)), 'user'+CONVERT(VARCHAR, @cnt), '1234', 'user'+CONVERT(VARCHAR, @cnt)+'@gmail.com', GETDATE(), 
		CASE 
...
WHILE @patient <= 20
BEGIN
	INSERT INTO [PATIENT] VALUES(@usrid,'Patient' + CONVERT(VARCHAR, @patient), 'B19060' + CONVERT(VARCHAR, @patient),
	CASE 
          WHEN RAND() < 0.3333 THEN 'Male'
          WHEN RAND() < 0.6666 THEN 'Female'
          ELSE 'Other'
    END, '0946' + CONVERT(VARCHAR, FLOOR(RAND()*(999999-100000+1)+100000)), CONVERT(DATE, GETDATE()), 'Can Tho City', 'Admin', GETDATE(),'Admin', GETDATE(), 'False')
